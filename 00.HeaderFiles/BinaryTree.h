...
public:
	BinaryTree();
	BinaryTree(const DT& root);
	BinaryTree(const DT& root, AbstractBinaryTree<DT>* left, AbstractBinaryTree<DT>* right);
	bool isEmpty();
	~BinaryTree();
	int height();
	int size();
	DT& root();
	void setRoot(const DT& data);
...
template<class DT>
BinaryTree<DT>::BinaryTree() : _rootData(NULL), _left(NULL), _right(NULL) {};
template<class DT>
BinaryTree<DT>::BinaryTree(const DT& root) : BinaryTree(root, NULL, NULL) {};
template<class DT>
BinaryTree<DT>::BinaryTree(const DT& root, AbstractBinaryTree<DT>* left, AbstractBinaryTree<DT>* right)
{
	_rootData = NULL;
...
template<class DT>
DT& BinaryTree<DT>::root()
{
	if (isEmpty()) throw BinaryTreeEmptyTree();
	return *_rootData;
}
template<class DT>
void BinaryTree<DT>::setRoot(const DT& data)
{
	if (_rootData != NULL)
		delete _rootData;
	else
