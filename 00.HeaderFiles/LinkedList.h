...
public:
	LinkedList();
	LinkedList(const DT&);
	LinkedList(const DT&, LinkedList<DT>*);
	...
	DT& infoAt(int);
	void add(const DT&);
	void insertAt(const DT&, int);
	DT remove();
	DT remove(const DT&);
	DT removeAt(int);
	void operator=(const LinkedList<DT>&);
	virtual AbstractLinkedList<DT>* next();
};
...
template<typename DT> LinkedList<DT>::LinkedList(const DT& info) : LinkedList(info, NULL){}
// Constructs a new Node, dynamically (manually) stores 'info' deep-copy, and points to 'next'
template<typename DT> LinkedList<DT>::LinkedList(const DT& info, LinkedList<DT>* next)
{
	_info = new DT(info);
	if (_info == NULL) throw LinkedListMemory();
	_next = next;
...
template<typename DT> DT& LinkedList<DT>::find(const DT& key)
{
...
template<typename DT> void LinkedList<DT>::add(const DT& newObj)
{
...
template<typename DT> void LinkedList<DT>::insertAt(const DT& newObj, int position)
{
